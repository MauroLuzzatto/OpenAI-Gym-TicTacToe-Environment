...
def create_plot(player1_reward_array, player2_reward_array):
    ...
    plt.plot(range(len(player1_reward_array)), player1_reward_array, label = 'Reward Player 1')
    plt.plot(range(len(player2_reward_array)), player2_reward_array, label = 'Reward Player 2')
    plt.legend()
    plt.grid()
    plt.show()
        ...
        for _step in range(start, max_steps + start):
            ...
            if _step%2 == 0:
                ...
                while action not in action_space:
                    action = int(input("choose an action from {}: ".format(action_space))) 
                    print('Action:', action)
